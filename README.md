...
| `k8ify.prometheus.serviceMonitor.endpoint.tlsConfig.ca: ${SERVICE_MONITOR_CA}` | CA certificate for TLS. Default is `null`. |
| `k8ify.prometheus.serviceMonitor.endpoint.tlsConfig.cert: ${SERVICE_MONITOR_CERT}` | Client certificate for TLS. Default is `null`. |
| `k8ify.prometheus.serviceMonitor.endpoint.tlsConfig.insecureSkipVerify: "false"` | Whether to skip TLS certificate verification. Default is `false`. Default is `null`. |
| `k8ify.prometheus.serviceMonitor.endpoint.tlsConfig.keySecretValue: ${SERVICE_MONITOR_KEY}` | Client key for TLS, typically a reference to a secret.  Default is `null`. |
| `k8ify.prometheus.serviceMonitor.endpoint.tlsConfig.maxVersion: "TLS13"` | Maximum TLS version to use. Options are `TLS10`, `TLS11`, `TLS12`, `TLS13`.  Default is `null`. |
| `k8ify.prometheus.serviceMonitor.endpoint.tlsConfig.minVersion: "TLS13"` | Minimum TLS version to use. Options are `TLS10`, `TLS11`, `TLS12`, `TLS13`.  Default is `null`. |
